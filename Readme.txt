Project Assessment:
    ...
    Cross-Ride allows its users to register as drivers and/or riders.
     ...
     At the end of shared ride driver sends a request to the server with driver id, rider id, 
     the start time of shared ride, the end time of shared ride and the distance covered in
     kilometers.
...
Tasks:
    1) Increase unit test coverage to reach 60%, achieving more than 60% will only consume
     your valuable
     time without an extra score.  
    2) Cross-Ride APIs are developed by 
    .inexperienced developer and 
    contains functional/logical bugs. You need to find those issues and fix them and 
    write unit tests.
    3) Frontend team needs a new API to display top 5 drivers with a maximum
     duration of their shared rides and show average distance covered during that time.
...
Development Environment:
  MySQL:
    ...
    In case you need to update the Database, please create a new changeset file in
